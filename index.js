...
app.get("/read_user_jobs", async (req, res) => {
    try {
        ...
        const job_infos = await FormDataModel1.find({ email: userEmail });
        res.send(job_infos);
    } catch (err) {
        res.status(500).send(err);
    }
...
const ZahialgaSchema = new mongoose.Schema({
    jobId: { type: mongoose.Schema.Types.ObjectId, ref: 'FormDataModel1', required: true },
...
app.post("/api/update_schedule", async (req, res) => {
    ...
    try {
        ...
        const timesToTake = selectedTimeSlots.map(slot => slot.split(" : ")[0]);
...
app.post("/api/purchase", async (req, res) => {
    const { zaal_id, userEmail, selectedDates, totalPrice, purchaseTime } = req.body;
